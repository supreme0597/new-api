...
func GetPerfMetrics(modelName string, group string, startTs int64, endTs int64, timeField string) ([]PerfMetric, error) {
	...
	query := DB.Model(&PerfMetric{}).
		Where("model_name = ? AND "+timeColumn+" >= ? AND "+timeColumn+" <= ?", modelName, startTs, endTs)
	if group != "" {
...
func GetPerfMetricsSummaryAll(startTs int64, endTs int64, groups []string, timeField string) ([]PerfMetricSummary, error) {
