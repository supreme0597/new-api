...
func RecordErrorLog(c *gin.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeSeconds int,
	...
	requestTime int64, modelStartTime int64, modelEndTime int64) *Log {
...
func RecordTaskBillingLog(params RecordTaskBillingLogParams) {
	...
	// 虚拟用户ID -999 表示性能采样任务
	if params.UserId == TestChannelOwnerUserId {
		username = "system"
	}
	tokenName := ""
	if params.TokenId == -1 {
...
func GetAllLogs(logType int, timeField string, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, group string, requestId string, upstreamRequestId string, sessionId string) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
	...
	timeColumn := "logs.created_at"
	switch timeField {
	case "model_start_time":
		timeColumn = "logs.model_start_time"
	case "model_end_time":
		timeColumn = "logs.model_end_time"
	case "request_time":
		timeColumn = "logs.request_time"
	}
	if startTimestamp != 0 {
		tx = tx.Where(timeColumn+" >= ?", startTimestamp)
	}
	if endTimestamp != 0 {
		tx = tx.Where(timeColumn+" <= ?", endTimestamp)
	}
	if channel != 0 {
	...
	selectFields := "id, user_id, created_at, type, content, username, token_name, model_name, " +
		...
		"request_time, model_start_time, model_end_time"
...
func GetUserLogs(userId int, logType int, timeField string, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, group string, requestId string, upstreamRequestId string, sessionId string) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
	if logType == LogTypeUnknown {
